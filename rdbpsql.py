...
class RdbPSql(RdbBase):
    ...
    def exec_querry(self, sql: str, return_type: str = '3'):
        # 打开数据库连接，执行sql，有返回值
        print('执行的是瀚高数据库', self.host, self.port, self.database, self.user, self.passwd)
        db = psycopg2.connect(host=self.host, port=int(self.port), user=self.user, password=self.passwd, database=self.database)
        # db = psycopg2.connect(database="davinci", user="postgres", password="postgres", host="10.64.14.67", port="5432")
        # 使用cursor()方法获取操作游标
        cursor = db.cursor()
        ...
        # print('@@@@@@@@@@@@@@',results,data,return_type,ret,info)
